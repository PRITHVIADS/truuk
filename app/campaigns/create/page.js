...
import AdvertiserSelect from "@/components/ui/AdvertiserSelect";
...
const MACRO_GROUPS=[{label:"Most Used",color:"orange",tokens:["{click_id}","{publisher_id}","{camp_id}","{source}","{p1}","{p2}","{sale_amount}","{payout}"]},{label:"Campaign",color:"blue",tokens:["{camp_id}","{campaign_title}","{adv_id}","{advertiser_name}"]},{label:"Publisher",color:"green",tokens:["{publisher_id}","{aff_id}","{aff_name}","{source}","{sub_source}"]},{label:"Click & Device",color:"purple",tokens:["{click_id}","{click_datetime}","{device}","{os}","{ip}","{country_id}","{region}","{referer}"]},{label:"Mobile IDs",color:"pink",tokens:["{gaid}","{idfa}","{android_id}","{fbclid}","{gclid}"]},{label:"Custom Params",color:"yellow",tokens:["{p1}","{p2}","{p3}","{p4}","{p5}","{sub1}","{sub2}","{sub3}","{sub4}","{sub5}"]},{label:"Random",color:"slate",tokens:["{random}","{random4}","{random_100}","{unix_time_stamp}"]}];
...
export default function CreateCampaign(){
  ...
  const handleSubmit=async()=>{
    ...
    if(!selectedAdvertiser){setError("Please select an advertiser");return;}
    ...
    <div className="max-w-4xl pb-10">
      ...
      <div className="rounded-2xl border overflow-hidden mb-4" style={card}>
        ...
        <div className="px-6">
          ...
          <div className={row}>
            <div><p className={lbl}>Advertiser <span className="text-red-400">*</span></p><p className="text-xs text-slate-600 mt-0.5">Select from database</p></div>
            <div className="md:col-span-2"><AdvertiserSelect value={selectedAdvertiser} onChange={setSelectedAdvertiser}/></div>
          </div>
          <div className={row}><p className={lbl}>Title <span className="text-red-400">*</span></p><div className="md:col-span-2"><input value={name} onChange={e=>setName(e.target.value)} className="input" placeholder="Campaign title"/></div></div>
          ...
          <div className={row}>
            <div><p className={lbl}>Campaign URL <span className="text-red-400">*</span></p><p className="text-xs text-slate-600 mt-0.5">Macros replaced on redirect</p></div>
            <div className="md:col-span-2">
              ...
              <div className="rounded-xl border border-white/10 overflow-hidden mb-2">
                <div className="flex items-center gap-1 px-3 py-2 border-b border-white/5 flex-wrap" style={{background:"rgba(0,0,0,0.3)"}}>
                  <span className="text-xs text-slate-500 font-semibold mr-1">Macros:</span>
              ...
              {landingUrl&&<div className="p-2 rounded-lg bg-black/30 border border-white/5 mb-2"><p className="text-xs text-slate-500 mb-1">Preview:</p><code className="text-xs text-green-400 break-all">{landingUrl.replace("{click_id}","clk_abc123").replace("{publisher_id}","pub456").replace("{p1}","social").replace("{sale_amount}","5000")}</code></div>}
              <div className="p-3 rounded-xl border border-blue-500/20 bg-blue-500/5"><p className="text-xs text-blue-400 font-bold mb-1">📬 Advertiser Postback URL</p><code className="text-xs text-slate-300 break-all">{typeof window!=="undefined"?window.location.origin:"https://truuk.vercel.app"}/api/conversions?cid=CAMP_ID&aid={"{publisher_id}"}&txid={"{txn_id}"}{objective==="Sale"?"&sale_amount={sale_amount}":""}</code></div>
            </div>
          </div>
      ...
      <div className="rounded-2xl border overflow-hidden mb-4" style={card}>
        ...
        <div className="px-6">
          ...
          <div className={row}><p className={lbl}>Currency</p><div className="md:col-span-2"><select value={currency} onChange={e=>setCurrency(e.target.value)} className="select">{CURRENCIES.map(c=><option key={c.code} value={c.code}>{c.symbol} {c.code}</option>)}</select></div></div>
          {objective==="Conversions"&&<>
            <div className={row}><div><p className={lbl}>Revenue <span className="text-red-400">*</span></p><p className="text-xs text-slate-600 mt-0.5">Charged from advertiser</p></div><div className="md:col-span-2"><div className="flex"><span className="flex items-center px-3 bg-white/5 border border-white/10 border-r-0 rounded-l-xl text-sm text-slate-400 font-semibold">{currency}</span><input type="number" value={flatRevenue} onChange={e=>setFlatRevenue(e.target.value)} className="input rounded-l-none flex-1" placeholder="Amount charged to advertiser"/></div></div></div>
            <div className={row}><p className={lbl}>Geo Coverage</p><div className="md:col-span-2"><div className="flex flex-wrap gap-2 mb-2">{geo.map(g=><span key={g} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-500/20 border border-blue-500/30 text-blue-400 text-xs font-bold">{g}<button onClick={()=>removeGeo(g)} type="button"><X size={10}/></button></span>)}</div><div className="flex gap-2 mb-2"><input value={geoInput} onChange={e=>setGeoInput(e.target.value.toUpperCase().slice(0,3))} onKeyDown={e=>{if(e.key==="Enter"||e.key===",")addGeo(geoInput);}} className="input flex-1 text-xs" placeholder="Start typing GEOs... e.g. IN, US, ALL"/><button onClick={()=>addGeo(geoInput)} type="button" className="btn-primary px-3 py-2 text-xs"><Plus size={14}/></button></div><div className="flex flex-wrap gap-1.5">{["ALL","IN","US","GB","AE","AU","CA","SG","MY"].map(g=><button key={g} onClick={()=>addGeo(g)} type="button" className={`px-2 py-1 rounded-lg border text-xs font-bold transition-all ${geo.includes(g)?"border-blue-500/30 bg-blue-500/10 text-blue-400":"border-white/10 bg-white/5 text-slate-400 hover:border-white/20"}`}>{g}</button>)}</div></div></div>
            <div className={row}><div><p className={lbl}>Payout <span className="text-red-400">*</span></p><p className="text-xs text-slate-600 mt-0.5">Pay to publisher</p></div><div className="md:col-span-2"><div className="flex"><span className="flex items-center px-3 bg-white/5 border border-white/10 border-r-0 rounded-l-xl text-sm text-slate-400 font-semibold">{currency}</span><input type="number" value={flatPayout} onChange={e=>setFlatPayout(e.target.value)} className="input rounded-l-none flex-1" placeholder="Amount paid to publisher"/></div>{flatRevenue&&flatPayout&&<p className="text-xs text-green-400 mt-1.5">✓ Margin: {sym}{(+flatRevenue - +flatPayout).toFixed(2)} per conversion</p>}</div></div>
          </>}
          {objective==="Sale"&&<>
            <div className={row}><div><p className={lbl}>Revenue % <span className="text-red-400">*</span></p><p className="text-xs text-slate-600 mt-0.5">% of sale charged to advertiser</p></div><div className="md:col-span-2 flex gap-2 items-center"><input type="number" value={saleRevenuePct} onChange={e=>setSaleRevenuePct(e.target.value)} className="input flex-1" placeholder="e.g. 15" min="0" max="100"/><span className="text-slate-400 font-bold text-xl">%</span></div></div>
            <div className={row}><p className={lbl}>Geo Coverage</p><div className="md:col-span-2"><div className="flex flex-wrap gap-2 mb-2">{geo.map(g=><span key={g} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-500/20 border border-blue-500/30 text-blue-400 text-xs font-bold">{g}<button onClick={()=>removeGeo(g)} type="button"><X size={10}/></button></span>)}</div><div className="flex gap-2 mb-2"><input value={geoInput} onChange={e=>setGeoInput(e.target.value.toUpperCase().slice(0,3))} onKeyDown={e=>{if(e.key==="Enter")addGeo(geoInput);}} className="input flex-1 text-xs" placeholder="Type GEO code…"/><button onClick={()=>addGeo(geoInput)} type="button" className="btn-primary px-3 py-2 text-xs"><Plus size={14}/></button></div><div className="flex flex-wrap gap-1.5">{["ALL","IN","US","GB","AE","AU","CA","SG"].map(g=><button key={g} onClick={()=>addGeo(g)} type="button" className={`px-2 py-1 rounded-lg border text-xs font-bold ${geo.includes(g)?"border-blue-500/30 bg-blue-500/10 text-blue-400":"border-white/10 bg-white/5 text-slate-400"}`}>{g}</button>)}</div></div></div>
            <div className={row}><div><p className={lbl}>Commission % <span className="text-red-400">*</span></p><p className="text-xs text-slate-600 mt-0.5">% of sale paid to publisher</p></div><div className="md:col-span-2"><div className="flex gap-2 items-center"><input type="number" value={saleCommissionPct} onChange={e=>setSaleCommissionPct(e.target.value)} className="input flex-1" placeholder="e.g. 10" min="0" max="100"/><span className="text-slate-400 font-bold text-xl">%</span></div>{saleRevenuePct&&saleCommissionPct&&<p className="text-xs text-green-400 mt-1.5">✓ Margin: {(+saleRevenuePct - +saleCommissionPct).toFixed(1)}%</p>}</div></div>
