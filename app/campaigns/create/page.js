      ...
      <div className="rounded-2xl border overflow-hidden mb-4" style={card}>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="p-4 rounded-xl border border-white/10 bg-white/5 space-y-4">
            <div className="flex items-center gap-2"><Shield size={16} className="text-red-400"/><p className="text-sm text-white font-bold">Block Traffic Sources</p>{blockedReferrers.length>0&&<span className="text-xs bg-red-500/20 text-red-400 border border-red-500/30 px-2 py-0.5 rounded-full">{blockedReferrers.length} blocked</span>}</div>
            <div className="grid grid-cols-2 gap-3">{[["302","🔀 302 Redirect","Redirect to another URL"],["200","⬛ 200 Blank Page","Empty page, click still fires"]].map(([val,label,desc])=><button key={val} onClick={()=>setBlockMode(val)} type="button" className={`p-3 rounded-xl border text-left transition-all ${blockMode===val?"border-red-500 bg-red-500/10":"border-white/10 bg-white/5"}`}><p className={`text-xs font-black ${blockMode===val?"text-red-400":"text-white"}`}>{label}</p><p className="text-xs text-slate-500 mt-0.5">{desc}</p></button>)}</div>
            {blockMode==="302"&&<div><label className="text-xs text-slate-400 font-semibold block mb-1.5">Redirect blocked traffic to:</label><input value={blockRedirectUrl} onChange={e=>setBlockRedirectUrl(e.target.value)} className="input text-sm" placeholder="https://google.com"/></div>}
            ...
            <div className="flex gap-2"><input value={newBlockDomain} onChange={e=>setNewBlockDomain(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")addBlockDomain();}} className="input flex-1 text-sm" placeholder="e.g. facebook.com"/><button onClick={addBlockDomain} type="button" className="btn-primary px-3 py-2 text-xs flex items-center gap-1"><Plus size={14}/>Add</button></div>
            <div><p className="text-xs text-slate-500 mb-2">Quick add:</p><div className="flex flex-wrap gap-1.5">{POPULAR_BLOCKS.filter(d=>!blockedReferrers.includes(d)).map(d=><button key={d} onClick={()=>setBlockedReferrers(p=>[...p,d])} type="button" className="px-2 py-1 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-400 hover:border-red-500/30 hover:text-red-400 transition-all">+ {d}</button>)}</div></div>
          </div>
        </div>
      </div>
      <div className="rounded-2xl border overflow-hidden mb-4" style={card}>
