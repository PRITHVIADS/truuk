...
export async function GET(req){try{const session=await getServerSession(authOptions);if(!session)return NextResponse.json({error:"Unauthorized"},{status:401});await connectDB();const{searchParams}=new URL(req.url);const range=parseInt(searchParams.get("range")||"30");const role=session.user.role;const userId=session.user.id;const since=new Date(Date.now()-range*24*60*60*1000);const query={createdAt:{$gte:since},isBot:{$ne:true}};
...
const clicks=await Click.find(query).select("referer campaignId affiliateId createdAt _id").lean();
const convs=await Conversion.find({createdAt:{$gte:since}}).select("payout clickId").lean();
const dm={};
