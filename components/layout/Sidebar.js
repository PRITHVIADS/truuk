...
function NavItem({ item, depth=0 }) {
    ...
    return (
      <div>
        <button onClick={() => setOpen(p=>!p)}
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive ? "text-orange-400 bg-orange-500/10" : "text-slate-500 hover:text-white hover:bg-white/5"}`}>
          ...
          <div className="ml-6 mt-0.5 space-y-0.5 border-l border-white/5 pl-3">
            {item.children.map(child => (
              <Link key={child.href} href={child.href}
                className={`block px-3 py-2 rounded-lg text-xs font-semibold transition-all ${pathname===child.href || pathname.startsWith(child.href) ? "text-orange-400 bg-orange-500/10" : "text-slate-500 hover:text-white hover:bg-white/5"}`}>
                {child.label}
              </Link>
            ))}
          </div>
  ...
  return (
    <Link href={item.href}
      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${active ? "bg-orange-500/15 text-orange-400" : "text-slate-500 hover:text-white hover:bg-white/5"}`}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-600 text-xs capitalize">{role} panel</p>
            </div>
          </div>
        ...
        <div className="px-3 py-3 border-t border-white/5">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-slate-600 text-xs capitalize">{role}</p>
            </div>
            <button onClick={()=>signOut({callbackUrl:"/login"})} className="text-slate-600 hover:text-red-400 transition-colors"><LogOut size={15}/></button>
          </div>
        </div>
      </aside>
    </>
